...
// curations, doodles) lives only in data.js — carry it over so a rebuild
// never clobbers in-app work.
function readExisting() {
  if (!existsSync(OUT_PATH)) return { places: [], zones: [], curations: [], doodles: [] };
  try {
    const w = {};
    new Function("window", readFileSync(OUT_PATH, "utf8"))(w);
    return { places: [], zones: [], curations: [], doodles: [], ...w.NIPPON };
  } catch (e) {
...
`;
  ...
  console.log(`\nWrote ${OUT_PATH}: ${places.length} places, ${CHAINS.length} chains, ${zones.length} zones, ${existing.curations.length} curations, ${existing.doodles.length} doodles`);
