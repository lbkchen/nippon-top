...
window.NIPPON = ${JSON.stringify({ places, chains: CHAINS, zones: ZONES }, null, 2)};
`;
  writeFileSync(OUT_PATH, out);
  console.log(`\nWrote ${OUT_PATH}: ${places.length} places, ${CHAINS.length} chains, ${ZONES.length} zones`);
  console.log(`Geocoded: ${places.filter((p, i) => finalCache[PLACES[i].id]?.coords).length}, fallback: ${places.filter((p, i) => !finalCache[PLACES[i].id]?.coords).length}, approx-flagged: ${places.filter((p) => p.approx).length}`);
}
